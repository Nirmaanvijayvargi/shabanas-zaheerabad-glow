  ...
  <section id="gallery" className="py-20 bg-gradient-section">
    <div className="container mx-auto px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
        <h2 className="font-serif text-3xl sm:text-4xl font-bold text-foreground mb-2">Real Results. Real Clients.</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {images.map((img, i) => (
          ...
          >
            <img src={img} alt={`Salon work showcase ${i + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
          </motion.a>
        ))}
      </div>

      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mt-8">
        <p className="text-muted-foreground mb-4">Follow us on Instagram for daily beauty inspiration</p>
        <a
          href={INSTAGRAM_URL}
          target="_blank"
          rel="noopener noreferrer"
